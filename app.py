# app.py — Buffett‑Style Value Screener (Streamlit • iPhone/iPad friendly)
...
# -------------------- Constants & polite headers --------------------
SEC_BASE = "https://data.sec.gov"
# 👉 Use your email so SEC allows polite scraping
HEADERS = {"User-Agent": "BuffettValueAnalyst/1.1 (Yeap Teck Hooi yeapteckhooi@gmail.com)"}

# -------------------- Tiny helpers --------------------
def pct(a, b):
    """a/b as ratio; None if invalid."""
    if a is None or b in (None, 0): return None
    try: return float(a) / float(b)
    except: return None

def cagr(v0, vN, years):
    """Compound annual growth rate with guards."""
    if v0 in (None, 0) or vN is None or not years or years <= 0: return None
    try: return (float(vN) / float(v0)) ** (1.0 / float(years)) - 1.0
    except: return None
...
def fetch_json(url: str) -> dict:
    last_err = None
    for i in range(5):  # up to 5 tries with backoff
        try:
            req = urllib.request.Request(url, headers=HEADERS)
            with urllib.request.urlopen(req, timeout=30) as r:
                return jsonlib.loads(r.read().decode("utf-8"))
        except urllib.error.HTTPError as e:
            # Fast‑fail on 404; backoff on 403/429/503
            if e.code == 404:
            ...
            last_err = e
            continue
    # If all retries fail, surface the last error
    raise last_err
...
def load_ticker_map() -> Dict[str, dict]:
    ...
    1) Try modern XBRL endpoint
    2) Fallback to legacy file
    3) Final tiny built‑in set so the app still works when SEC rate‑limits
    """
    sources = [
    ...
    st.warning(
        "SEC ticker map unavailable — using a limited built‑in map. "
        "Try again later for full coverage."
    )
    return {
        ...
        "ADBE": {"ticker": "ADBE", "cik_str": 796343},
    }

@st.cache_data(ttl=3600, show_spinner=False)
def get_company_facts(cik: str) -> dict:
    """SEC XBRL company facts (CIK must be zero‑padded)."""
...
def pick_usd_facts(facts, concept):
    ...
    for unit, items in units.items():
        if unit in ("USD", "USD/shares", "shares", "pure"):
            out += items
    return out
...
def rollup_annual(items):
    out = {}
    for it in items:
        if it.get("fp") not in ("FY", None):
            continue
        if it.get("form") not in ("10-K", "20-F", "40-F", "10-K/A", "20-F/A", "40-F/A"):
            continue
...
def choose_10y_plus_ttm(annual, ttm):
    years = sorted(annual.keys())[-10:]
    return years, [annual.get(y) for y in years], ttm

def invested_capital(assets, equity, cash, dl, ds):
    if equity is None:
        return None
    ic = (dl or 0) + (ds or 0) + equity - (cash or 0)
    return ic if ic and ic > 0 else None

def interest_cov(ebit, ix):
    if ebit is None or not ix:
        return None
    return ebit / abs(ix)

# -------------------- Bundle & Metrics --------------------
def pull_bundle(facts):
    # Revenue special handling (common alt concept)
    ...
    gpA, cogsA, opA, ptxA, niA, cfoA, capexA = (
        A(KEY["GrossProfit"]), A(KEY["COGS"]), A(KEY["OperatingIncome"]),
        A(KEY["PretaxIncome"]), A(KEY["NetIncome"]), A(KEY["CFO"]), A(KEY["Capex"])
    )
    shB, shD = A(KEY["SharesBasic"]), A(KEY["SharesDiluted"])
    ixA, cashA, dlA, dsA, eqA, asA, segA = (
        A(KEY["InterestExpense"]), A(KEY["Cash"]), A(KEY["DebtLT"]),
        A(KEY["DebtST"]), A(KEY["Equity"]), A(KEY["Assets"]), A(KEY["Segments"])
    )
    ...
    return {
        ...
        "ttm": {
            "revenue": rev_ttm, "gross_profit": T(KEY["GrossProfit"]),
            "operating_income": T(KEY["OperatingIncome"]),
            "net_income": T(KEY["NetIncome"]),
            "cfo": T(KEY["CFO"]), "capex": T(KEY["Capex"]),
            "interest_expense": T(KEY["InterestExpense"]),
        }
    }
...
def compute_metrics(b):
    ...
    # ROIC ≈ NOPAT / Invested Capital; NOPAT ~ EBIT*(1‑tax), use 21% baseline
    roic = []
    for i in range(n):
        ic = invested_capital(
            assets[i] if i < len(assets) else None,
            eq[i] if i < len(eq) else None,
            cash[i] if i < len(cash) else None,
            dl[i] if i < len(dl) else None,
    ...
    s = safe(fcf)
    fcf_vol = (statistics.pstdev(s) / abs(statistics.mean(s))) if (len(s) >= 5 and statistics.mean(s) != 0) else None

    return {
        "gm": gm, "opm": opm, "fcf": fcf, "fcf_margin": fcf_m, "roic": roic,
        "net_debt_to_ebit": nd_to_ebit, "interest_coverage": cov,
        "fcf_over_ni": fcf_over_ni, "rev_cagr_10y": rev_cagr_10,
        "fcf_cagr_5y": fcf_cagr_5, "fcf_vol": fcf_vol
    }
...
def owner_earnings_dcf(fcf_series, shares_series, price, discount, terminal_g, mos):
    ...
    if not sfcf:
        return {"iv_ps": None, "iv_ps_mos": None, "pass": False}
    fcf0 = sfcf[-1]
    sh = next((s for s in reversed(shares_series or []) if s), None)
    if not sh or sh <= 0:
        return {"iv_ps": None, "iv_ps_mos": None, "pass": False}

    def cap_growth(g5, g10):
        return max(min(g5 or 0.0, (g10 or 0.0) * 0.75), 0.0)
    ...
    g_base = cap_growth(g5, g10)
    g_bear = max(g_base - 0.04, 0.0)
    g_bull = g_base + 0.04
    ...
    return {
        "iv_ps": float(ivps),
        "iv_ps_bear": float(ivbear / sh) if ivbear else None,
        "iv_ps_bull": float(ivbull / sh) if ivbull else None,
        "iv_ps_mos": float(ivps_mos),
        "pass": bool(pass_mos),
    }

# -------------------- Price (Yahoo + HTTP fallback) --------------------
def yahoo_price(ticker) -> Optional[float]:
    try:
        ...
        with urllib.request.urlopen(req, timeout=20) as r:
            ...
            if res and "regularMarketPrice" in res[0]:
...
def scorecard(m, b, mos_ok, min_roic=0.15, min_cov=6.0, max_nd_ebit=3.0, min_cash_conv=0.85):
    ...
    segs = [s for s in b["segments"] if s is not None]
    complex_biz = (max(segs) if segs else 1) > 3

    return {
        "L1": not complex_biz,
        "L2": (roic_med is not None and roic_med >= min_roic and not gm_decl),
        "L3": (fcfni5 is not None and fcfni5 >= min_cash_conv),
        "L4": ((m["net_debt_to_ebit"] is None or m["net_debt_to_ebit"] < max_nd_ebit) and
               (m["interest_coverage"] is None or m["interest_coverage"] > min_cov)),
        "L5": True,             # Mgmt/capital allocation – manual later
        "L6": bool(mos_ok),     # Valuation/MOS
        "L7": True              # Risks/Catalysts placeholder
    }
...
def analyze_one(t):
    ...
    if not tmap:
        return {"ticker": t, "error": "SEC ticker map unavailable (rate‑limit)."}
    if t not in tmap:
        return {"ticker": t, "error": "Not an SEC XBRL filer or ticker not found"}
    ...
    m = compute_metrics(b)
    price = yahoo_price(t)
    dcf = owner_earnings_dcf(
        m["fcf"], b["shares_diluted"] or b["shares_basic"], price,
        discount, terminal, mos
    )
    sc = scorecard(
        m, b, dcf["pass"],
        min_roic=min_roic, min_cov=min_cov,
        max_nd_ebit=max_nd_ebit, min_cash_conv=min_cash_conv
    )
...
if run:
    ...
    for i, t in enumerate(tickers, start=1):
        try:
            ...
            results.append({"ticker": t, "error": f"Unhandled error: {e}"})
    ...
    for r in results:
        ...
        rows.append({
            ...
            "Pass_L1": sc["L1"], "Pass_L2": sc["L2"], "Pass_L3": sc["L3"],
            "Pass_L4": sc["L4"], "Pass_L5": sc["L5"], "Pass_L6": sc["L6"],
            "Pass_L7": sc["L7"], "AllPass": all(sc.values()),
        })
    ...
    for r in results:
        if "error" in r:
            with st.expander(f"{r['ticker']} — Error"): st.error(r["error"])
            continue
        ...
        with st.expander(f"{r['ticker']} — audit & charts"):
            ...
            c1, c2 = st.columns(2)
            with c1:
                st.plotly_chart(px.line(chart_df, x="Year", y="FCF", markers=True, title="Free Cash Flow"),
                                use_container_width=True)
            with c2:
                st.plotly_chart(px.line(chart_df, x="Year", y="ROIC", markers=True, title="ROIC"),
                                use_container_width=True)

            st.caption(
                "ROIC≈NOPAT/(Equity+Debt−Cash); ND/EBIT used as conservative proxy for ND/EBITDA. "
                "Treat as prefilter; audit in filings."
            )
